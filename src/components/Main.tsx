        ...
        <section className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2">
                <article className="lg:p-8">
                    <div className="relative">
                        <img alt="product" onClick={() => setLightBoxOpen(true)} src={mainImage} className="cursor-pointer w-full lg:rounded-xl"/>

                        
                        <div className="absolute lg:hidden top-1/2 -translate-y-1/2 w-full flex items-center justify-between px-4">
                            <button title="left" onClick={previousImg} className="text-xl bg-white rounded-full p-3"><AiOutlineLeft/></button>
                            <button title="right" onClick={nextImg} className="text-xl bg-white rounded-full p-3"><AiOutlineRight/></button>
                        </div>
                    </div>
                    <div className="hidden lg:flex  items-center justify-between  mt-4">
                        {products.map((product, i) => {
                            return  <div className={`rounded-lg border-2 duration-200  overflow-hidden  cursor-pointer ${index === i && " border-orange-400"}`} onClick={() => setIndex(i)} >
                                        <img alt="product" className={`w-16 duration-200 hover:opacity-40 ${index === i && "opacity-40"}`}  key={i} src={product.mainImage}/>
                                    </div>
                        })} 
                    </div>
                </article>
                ...
                <article className="p-8 flex flex-col items-start justify-center gap-4">
                    ...
                    <div className="flex flex-col lg:flex-row items-center gap-6 w-full">
                        <div className="flex w-full lg:w-min  items-center justify-between gap-8 rounded-md bg-grayishBlue bg-opacity-20 p-3">
                            <button title="minus" onClick={() => N > 1 ? setN(N-1) : null} className="text-orange-400 hover:text-orange-200 duration-200"><FaMinus/></button>
                            <p className="font-bold">{N}</p>
                            <button title="plus" onClick={() => setN(N+1)} className="text-orange-400 hover:text-orange-200 duration-200"><FaPlus/></button>
                        </div>
